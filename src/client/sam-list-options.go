...
func SetTimeout(s int) func(*SamList) error {
	return func(c *SamList) error {
		if s > 5 {
			...
			return fmt.Errorf("A specified timeout must be less than a specified lifetime. %s %s", strconv.Itoa(s), strconv.Itoa(c.lifeTime))
		}
...
func SetLifespan(s int) func(*SamList) error {
	return func(c *SamList) error {
		...
		return fmt.Errorf("A specified lifetime must be greater than a specified timeout. %s %s", strconv.Itoa(s), strconv.Itoa(c.timeoutTime))
	}
}
